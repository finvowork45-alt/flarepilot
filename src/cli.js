...
var dbCmd = program
  .command("db")
  .description("Manage app database (D1)");
...
dbCmd
  .command("create [name]")
  .description("Create and attach a D1 database")
  .option("--location <hint>", "D1 location hint (wnam, enam, weur, eeur, apac)")
  .option("--jurisdiction <j>", "D1 jurisdiction (eu, fedramp)")
  .option("--json", "Output as JSON")
  .action(dbCreate);

dbCmd
  .command("destroy [name]")
  .description("Destroy the attached D1 database")
  .option("--confirm <name>", "Confirm by providing the app name")
  .action(dbDestroy);
...
dbCmd
  .command("token [name]")
  .description("Show or rotate Hrana auth token")
